...
LLM_KEY   = os.getenv("LLM_KEY",       "")   # required — set in Render/Railway env vars
LLM_MODEL = os.getenv("LLM_MODEL",     "llama-3.1-8b-instant")
# Optional: comma-separated fallback keys for rate-limit resilience
_fb_raw   = os.getenv("LLM_KEY_FALLBACKS", "")
LLM_KEY_FALLBACKS = [k.strip() for k in _fb_raw.split(",") if k.strip()]
...
def llm_reply(user_msg: str) -> str:
    ...
    keys_to_try = [LLM_KEY] + LLM_KEY_FALLBACKS
    for key in keys_to_try:
        ...
        try:
            ...
            if e.code == 429:
                log.warning(f"Rate limit on key ...{key[-6:]}, trying next")
                continue
            log.warning(f"LLM HTTP {e.code}: {e.read()[:200]}")
            return None
